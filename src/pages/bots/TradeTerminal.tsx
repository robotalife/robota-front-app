...
const TradeTerminal = () => {
  ...
  const [selectedBot, setSelectedBot] = useState<AutocompleteOption | null>(null);
  ...
  useEffect(() => {
    ...
    newSocket.onopen = () => {
      const tradingPair = findBotByBotId(selectedBot.value as string)?.tradingPair;
      newSocket.send(
        JSON.stringify({
          method: 'SUBSCRIBE',
  ...
  const getActiveTrades = useCallback(async (botId: string | undefined | null) => {
    try {
  ...
  return (
    <Container maxWidth="xl" sx={{ m: 0, padding: '0!important' }}>
            ...
            <WrapperBoxHeader
              title={
                selectedBot === null
                  ...
                  : `${findBotByBotId(selectedBot.value as string)?.name} - ${findBotByBotId(selectedBot.value as string)
                      ?.tradingPair} - ${findBotByBotId(selectedBot.value as string)?.strategy}`
              }
            />
            <WrapperBoxSection>
              ...
              {selectedBot && (
                <>
                  <Button
                    ...
                    size="small"
                    fullWidth
                    onClick={() => navigate(`${routes.botOverview}/${selectedBot?.value}`)}
                    sx={{ mb: 3, mt: 3 }}
                  >
                    View bot overview
                  </Button>
                  ...
                  <Button color={'success'} variant={'contained'} size={'small'} fullWidth onClick={openPosition} disabled={loading}>
      ...
      <WrapperBox>
        <WrapperBoxHeader title={selectedBot === null ? 'Select a bot to start trade' : 'Active Trade'} />
        {loading ? (
          <Loader />
        ) : (
          <Table>
