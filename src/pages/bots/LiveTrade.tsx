...
const LiveTrade = () => {
  ...
  const postTrade = useCallback(async (action: 'START' | 'STOP', botId: number) => {
    try {
      setLoading(true);
      const response: AxiosResponse<any, any> = await axios.post(apiEndPoints.signal, {
        action: action,
        botId: botId,
      });
      getLiveTrades();
  ...
  return (
    <WrapperBox>
      <WrapperBoxHeader title="Live Trade" />
      <WrapperBoxSection noPadding>
        {loading ? (
          ...
          <Table>
            ...
            <TableBody>
              {Array.isArray(liveTrades) && liveTrades.length ? (
                liveTrades.map((h, i) => (
                  <TableRow key={`${h.botName}_${h.botStrategy}_${i}`}>
                    <TableCell>
                      <Grid container spacing={1} alignItems={'center'} wrap="nowrap">
                        ...
                        <Grid item xs={'auto'}>
                          <TableDateTime date={h.botName} time={`${h.baseAsset}_${h.qouteAsset}`} />
                        </Grid>
                      </Grid>
                    </TableCell>
                    <TableCell>
