...
const AddExchangeCard = ({ title, description }: Props) => {
  ...
  return (
    <Box className={classes.exchangeCard}>
      <Grid container justifyContent={'space-between'} alignItems={'flex-start'} className={classes.borderBottom} sx={{ mb: 3 }}>
        <Grid item xs="auto">
      ...
      <Grid container justifyContent={'space-between'} alignItems={'center'}>
        <Grid item xs={12}>
          <Button variant="outlined" size="small" fullWidth onClick={() => navigate(`${routes.exchangeNew}?returnTo=${currentPage}`)}>
