...
const BotOverviewCard = ({ action, hasChart, content, description, title, chartData }: Props) => {
  ...
  return (
    <Grid container justifyContent={'space-between'}>
      <Grid item xs>
        ...
        {hasChart ? (
          <Typography className={`${isNegative ? classes.negativeContent : classes.positiveContent}`}>{content}</Typography>
        ) : (
      ...
      >
        ...
        <div className={classes.chartContainer}>{hasChart && <BotCardChart input={chartData ? chartData : emptyChart} />}</div>
      </Grid>
    </Grid>
  );
};
