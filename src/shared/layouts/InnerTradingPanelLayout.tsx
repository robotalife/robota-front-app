...
const InnerTradingPanelLayout = () => {
  ...
  const pageTitle = botTabs(botId).find((link) => pathname === link.to)?.label || '';

  const stopBot = useCallback(async () => {
    setLoading(true);

    try {
      const response: AxiosResponse<any, any> = await axios.post(apiEndPoints.getStopBot(botId as string));
      getBotData();
    } catch (error) {
      // Handle error
    } finally {
  ...
  const startBot = useCallback(async () => {
    ...
    try {
      const response: AxiosResponse<any, any> = await axios.post(apiEndPoints.getRestartBot(botId as string));
      getBotData();
    } catch (error) {
      // Handle error
    } finally {
  ...
  const deleteBot = useCallback(async (id: string) => {
    try {
      const response: AxiosResponse<any, any> = await axios.delete(`${apiEndPoints.bots}/${id}`);
        ...
        <PageTitle title={pageTitle}>
          <Grid container spacing={2} alignItems={'center'}>
            <Grid item xs={12} sx={{ display: { lg: 'none' } }}>
              <Button variant="contained" color="secondary" size="small" sx={{ mb: { xs: 2, lg: 0 } }} fullWidth onClick={() => setIsOpen(!isOpen)}>
            ...
            <Grid item xs={12} lg={'auto'}>
              <Grid container columns={2} spacing={2}>
                <Grid item xs={6} lg>
                  <Tooltip title="You can delete a bot if it's stopped and does not have any active trade.">
                    <div>
                      <Button
                        variant="outlined"
                        size="medium"
        ...
        <>
          ...
          <Grid container spacing={1.75}>
            <Grid item xs={12} lg={6} order={{ xs: 1, lg: 2 }}>
              <Button fullWidth variant="contained" color="error" onClick={() => deleteBot(botId)}>
                Delete
              </Button>
            </Grid>
