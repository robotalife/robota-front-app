...
export const StoreBotsProvider = ({ children }: PropsWithChildren) => {
  ...
  const [paginateData, setPaginateData] = useState<PaginationObj>({} as PaginationObj);
  ...
  const loadMyBots = useCallback(async () => {
    setLoading(true);
    try {
      const response: AxiosResponse<IMyBots, any> = await axios.get(`${apiEndPoints.storeBots}?${getFiltersString(filters)}`);
  ...
  return <StoreBotsContext.Provider value={value}>{children}</StoreBotsContext.Provider>;
};
